<template>
  <div class="prodetail">
    <van-nav-bar fixed title="商品詳情頁" left-arrow @click-left="$router.go(-1)" />
    ...
    <div class="comment">
      ...
      <div class="comment-list">
        <div class="comment-item" v-for="item in list" :key="item.comment_id">
          <div class="top">
            <img :src="item.user.avatar_url || deIMG" alt="">
    ...
    <div class="footer">
      ...
      <div @click="showcart" class="btn-add">加入購物車</div>
      <div @click="showbuy" class="btn-buy">立刻購買</div>
    </div>
...
import { GetdetailData, GetlistRowsData } from '@/api/search'
import deIMG from '@/assets/default-avatar.png'
import { GetCartAddData } from '@/api/cart'
export default {
  name: 'ProDetail',
  components: {
    CountBox
  ...
  methods: {
    ...
    // 評論API
    async getlistRows () {
      const { data: { list, total } } = await GetlistRowsData(this.goodId, 3)
      this.list = list
      this.total = total
    },
    showcart () {
